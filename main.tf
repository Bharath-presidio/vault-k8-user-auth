resource "vault_kubernetes_secret_backend" "config" {
  ...
  kubernetes_ca_cert        = file("${path.module}/kubernetes-ca-cert.pem")
}

resource "vault_kubernetes_secret_backend_role" "backend-role" {
  count                         = length(var.k8_service_accounts)
  ...
  service_account_name          = var.k8_service_accounts[count.index]
}